...
export default function FormRezerv() {
     ...
     const DateRezerv = useRef<HTMLInputElement>(null);
     const Ocloack = useRef<HTMLInputElement>(null);
     const NumberGuset = useRef<HTMLInputElement>(null);
     const router = useRouter();
     const seaveReze = () => {
          const Token = localStorage.getItem('token')
          Token && FerstName && LastName && PhoneNumber && DateRezerv && Ocloack && NumberGuset && axios.post('127.0.0.1:8000/api/restaurant/reserves', {
     ...
     const showListofTable = () => {
          
         if( FerstName.current?.value && LastName.current?.value && PhoneNumber.current?.value  && Ocloack.current?.value && NumberGuset.current?.value){
          setShowList(!showList)
          console.log(showList);
          console.log(FerstName.current.value);
          
         }
         else{
          console.log(FerstName.current?.value,LastName.current?.value,PhoneNumber.current?.value, DateRezerv.current?.value , Ocloack.current?.value);
          setShowList(false)
          Swal.fire({
     ...
     return (
          <>
               <form onSubmit={(e) => e.preventDefault()} className="max-w-2xl mx-auto my-10 p-5 bg-white dark:bg-slate-900">
                    ...
                    <button onClick={showListofTable} className="animate-bounce text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                         مشاهده میز ها</button>
               </form >
               {
                   showList &&<ListOfTableRest />
               }
