...
const EditMenuPage = ({ menuItemId }: { menuItemId: string }) => {
    ...
    if (isError)
        return <div className='text-center flex flex-col gap-6 min-h-[620px] p-4 text-2xl font-bold'>میزی پیدا نشد
            <Link href='/resturant/admin' className='mx-auto bg-coffee w-fit xs:w-20 text-gray-200 text-lg px-4 py-2 inline-block animate-pulse rounded-lg'>بازگشت به پنل ادمین </Link>
        </div>

    return (
        <main className='min-h-[620px] py-4 flex flex-col gap-4 items-center bg-neutral-300 dark:bg-slate-800'>
            <Link href='/resturant/admin' className='absolute top-24 left-4 xs:w-20 text-gray-200 text-center bg-coffee text-lg px-4 py-2 inline-block animate-pulse rounded-lg'>بازگشت به پنل ادمین </Link>
