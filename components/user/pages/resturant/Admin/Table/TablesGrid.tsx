        ...
        tables?.length === 0 ? <h2 className=' text-center font-bold text-xl py-8 animate-pulse'>در حال حاضر میز فعالی وجود ندارد </h2> :
            <div className='max-w-3xl mx-auto md:overflow-x-scroll w-11/12'>
                <table className='px-6 py-10 dark:bg-slate-700 w-full text-center '>
                    ...
                    <th className='align-middle border-2 px-2 py-1  border-indigo-400'>شماره میز</th>
                    <th className='align-middle border-2 px-2 py-1 border-indigo-400'>ظرفیت میز</th>
                    <th className='align-middle border-2 px-2 py-1 border-indigo-400'>نوع میز</th>
                    <th className='align-middle border-2 px-2 py-1 border-indigo-400'>قیمت رزرو</th>
                    {/* <th className='border border-indigo-400 block sm:hidden'>توضیحات</th> */}
                    <th className='align-middle border-2 px-2 py-1 border-indigo-400'>فعال </th>
                    <th className='align-middle border-2 px-2 py-1 border-indigo-400' colSpan={0.3} >ویرایش و حذف</th>
                    {tables.map((table) => <tr key={table.id}>
                        ...
                        <td className='border-2 border-indigo-400 font-semibold '>
                            <div className='flex justify-center items-center gap-6 xs:gap-2'>
                            <Link href={`/resturant/admin/editTable/${table.id}`}>
                                <CiEdit className='cursor-pointer fill-green-700 dark:fill-[#0ecb11] ' size={24} />
                            </Link>
                            <FaTrashAlt className='cursor-pointer' color='#e71212' size={20} onClick={() => deleteTable(table.id)} />
                            </div>
                            </td>
...
const TableData = ({ children, className }: { children: ReactNode, className?: string }) => {
    return <td className={`border-2 border-indigo-400 px-2 py-1 font-semibold ${className}`} >
        {children}
    </td>
}