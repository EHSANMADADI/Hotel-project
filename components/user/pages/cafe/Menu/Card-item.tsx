    ...
    return (<article className='flex xs:flex-col rounded-lg bg-stone-300/80 dark:bg-stone-950/80 justify-between px-4 lg:px-8 md:px-6 py-2 items-center'>
        ...
        <div className='flex items-center lg:gap-8 gap-6'>
            <div className='flex gap-2 items-center'>
                <button onClick={() => (dispatch({ type: "ADD", payload: { id, price, name, image } }))} className='bg-green-500 hover:bg-green-600 flex justify-center items-center font-semibold duration-200 w-8 h-8 rounded-full'>+</button>
                <p className='dark:text-white text-slate-900 font-semibold'>{selectedItem?.quantity || 0}</p>
                <button onClick={() => (dispatch({ type: "REMOVE", payload: { id, price, name, image } }))} className='bg-red-500 hover:bg-red-600 flex justify-center items-center font-semibold duration-200 w-8 h-8 rounded-full'> - </button>
            </div>
