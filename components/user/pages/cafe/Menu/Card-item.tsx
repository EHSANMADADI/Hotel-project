...
const CardItem = ({ item: { description, id, media: image, name, price } }: MenuItemProps) => {
    ...
    return (<article className='flex xs:flex-col rounded-lg bg-stone-300/80 dark:bg-stone-950/80 justify-between px-4 lg:px-8 md:px-6 py-2 items-center'>
        <div className='flex gap-4 items-center'>
            <Image className='rounded-full border-2 border-dashed border-coffee bg-coffee/25' src={`http://127.0.0.1:8000/menu-item/${image[0]?.url}` || ''} alt='' width={72} height={72} />
            <div>
        ...
        <div className='flex items-center lg:gap-8 gap-6'>
            <div className='flex gap-2 items-center'>
                <button onClick={() => (dispatch({ type: "ADD", payload: { id, price, name, image: (image[0]?.url || '') } }))} className='bg-green-500 hover:bg-green-600 flex justify-center items-center font-semibold duration-200 w-8 h-8 rounded-full'>+</button>
