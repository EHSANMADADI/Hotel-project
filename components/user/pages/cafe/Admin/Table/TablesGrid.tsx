...
const TablesGrid = ({ tables }: TableType) => {
    ...
    return (
        tables?.length === 0 ? <h2 className=' text-center font-bold text-xl py-8 animate-pulse'>در حال حاضر میز فعالی وجود ندارد </h2> :
            <table className='rounded-xl px-6 py-10 dark:bg-slate-700 w-full text-center '>
                ...
                {tables.map((table) => <tr key={table.id}>
                    ...
                    <td className='border border-indigo-400 font-semibold flex justify-center items-center gap-6'>
